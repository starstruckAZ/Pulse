...
export default function BlogPage() {
  return (
    <div className="relative min-h-screen overflow-hidden">
      <div className="mesh-gradient right-[-10%] top-[10%] h-[500px] w-[500px] bg-orange-500/6" />
      ...
      <section className="pb-24">
        <div className="mx-auto max-w-4xl px-6 space-y-4">
          {blogPosts.map((post) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className="bento group block p-8 transition hover:border-orange-500/20">
              <div className="mb-3 flex items-center gap-3">
                <span className="badge text-orange-400 border-orange-500/20 bg-orange-500/5 text-xs">{post.category}</span>
                <span className="flex items-center gap-1 text-xs text-zinc-600"><Clock className="h-3 w-3" />{post.readTime}</span>
                <span className="text-xs text-zinc-700">{post.date}</span>
              </div>
              <h2 className="mb-3 font-display text-xl font-bold transition group-hover:text-orange-400">{post.title}</h2>
              <p className="text-sm leading-relaxed text-zinc-400">{post.excerpt}</p>
              <span className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-orange-400 transition group-hover:gap-2">
      ...
      <section className="border-t border-white/5 py-16">
        <div className="mx-auto max-w-2xl px-6 text-center">
          ...
          <p className="mb-6 text-sm text-zinc-400">Weekly insights on review management and growing your local business.</p>
